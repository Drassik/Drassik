# 👋 Hi, I'm Boujendar Omar

Welcome to my GitHub profile!

---

## 🎓 Academic Background

### 🏫 University: Politcnico di Torino
- 📍 Turin, Italy
- 🎓 Degree: MSC in Data Science and engineering  (Double degree) 
- 📅 Graduation Year: 2027
- 📚 Relevant Coursework:
  - Data manageent and visualization
  - Data science and machine learning lab
  - Fundamentals of Artificial Intelligence, Machine and Deep Learning
    
### 🏫 Ecole d'ingénieur: Centrale Méditerranée
- 📍 Marseille, France
- 🎓 Cursus ingénieur grande école 
- 📅 Graduation Year: 2027
- 📚 Relevant Coursework:
  - Statistiques: estimations des modèles paramétriques et non paramétriques
  - Probabilités
  - Data programming (python, java)

### 🏫 Lycée Déodat de Séverac: CPGE PSI*
- 📍 Toulouse, Frnace
- 🎓 Classes Préparatoires aux grandes Ecoles, filière PSI* 
- 📅 Graduation Year: 2023
- 📚 Relevant Coursework:
  - Mathématiques: Proabilités et statistiques, analyse et algèbre
  - Physique: Magnétisme, optique, Gravitation, Electronique
  - Sciences de l'ingénieur: Etude de systèmes
---

## 💻 Programming Skills

### 🧠 Languages
- Python, Java, basics of C++, LaTeX

### ⚙️ Frameworks & Libraries
- Django, MongoDB, Tableau, Power BI

### 🛠️ Tools & Technologies
- Git, Linux

### 🗄️ Databases
- MySQL, Extended SQL, MongoDB

---

## 🚀 Projects

### 🔹 Completed Projects
- **[Volume estimation with Lidar]**
  - 📄 Description: Computing the volume of waste piles in hoppers for waste management plants
  - 🛠️ Tech Stack: [Python, Linux, C++, Git]

- **PM2.5 prediction in Turin**
  - 📄 Description: Establishing PM2.5 concentration for the upcoming week based on the weather and previous years' records
  - 🛠️ Tech Stack: [Python: Scikit-learn, PyTorch, numpy, pandas, matplotlib]
  - 🔗 Repo: [[Link](https://github.com/Drassik/PM2.5-Prediction)]

- **Newspaper classification**
  - 📄 Description: Classifying newspaper articles according to the outlet, title and text
  - 🛠️ Tech Stack: [Python: scikit-learn, numpy, pandas]
  - 🔗 Repo:


---

### 🔸 Upcoming / In Progress Projects
- **AWS Certification**
  - 💡Getting my basic AWS certification, possibly specializing in AWS for AI and ML
  - 🛠️ Planned Tech: [Technologies]

---

## 🌐 Connect With Me

- 💼 LinkedIn: [[Boujendar Omar](https://www.linkedin.com/in/omar-boujendar/)]
- 📧 Email: [omar.boujendar@centrale-med.fr]


---
